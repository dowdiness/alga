...
- **DFS event test helpers** — `dfs_events` tests repeat a count/filter-by-variant pattern 9 times across `src/dfs_test.mbt` and `src/graph_expr_qc.mbt`. If more `dfs_events`-consuming tests are added, extract shared helpers (e.g., `count_events(events, pred)`, `collect_edges(events, variant)`). Source: code review of `dfs_events`
